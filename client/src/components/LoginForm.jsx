  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-white">
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-medium text-xinc-800">{title}</h1>
          <p className="text-slate-500 text-sm sm:text-base mt-2">{subtitle}</p>
        </div>
        {error && (
          <div className="mb-6 p-6 bg-rose-50 border border-rose-200 test-rose-700 text-sm rounded-xl flex items-start gap-3">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div>
            ...
            <input 
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)} required placeholder="alishakumari72024@gmail.com"/>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" className=" absolute right-3 top-1/2 -translate-y1/2 text-slate-400 hover:text-slate-600 transition-colors" onClick={()=> setShowPassword(!showPassword)}>
